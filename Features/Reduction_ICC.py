...
df_all = pd.read_csv(root + "Aaron\ProstateMRL\Data\Paper1\\HM-FSTP\\Features\\Limbus_fts_l.csv")
...
print(fts_remove)
fts_remove.to_csv(root + "Aaron\ProstateMRL\Data\Paper1\\HM-FSTP\\Features\\FeaturesRemoved_ICC.csv", index = False)

# read in all features
df_all_r = pd.read_csv(root + "Aaron\ProstateMRL\Data\Paper1\HM-FSTP\\Features\\All_fts_l.csv")
df_all_r = df_all_r[~df_all_r["Feature"].isin(fts_remove["Feature"])]
df_all_r = df_all_r[df_all_r["Feature"] != "original_firstorder_Minimum"]
df_all_r = df_all_r[df_all_r["Feature"] != "original_firstorder_Maximum"]

# save df_all_r
df_all_r.to_csv(root + "Aaron\ProstateMRL\Data\Paper1\HM-FSTP\\Features\\All_fts_pICC.csv", index = False)