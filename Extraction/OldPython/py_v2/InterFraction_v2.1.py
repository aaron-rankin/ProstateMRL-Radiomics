...
col_names = ["PatID","Treatment","Date","MRScan","Normalisation","Region","DaysDiff","MeanSignal","StdSignal", "90thPerc"]
all_df = pd.DataFrame()
            ...
            for k in image_paths:
                ...
                if len(images) != 0:
                    ...
                    for m in masks:
                        ...
                        scan_mean, scan_std, scan_perc = UF.MaskedMeanStd90P(image_url, mask_url)
                        ...
                        vals_scan["Region"], vals_scan["MeanSignal"], vals_scan["StdSignal"], vals_scan["90thPerc"] = region, scan_mean, scan_std, scan_perc
                        vals_df = vals_df.append(vals_scan, ignore_index=True)
        if patID == "0001464":
            patID = patID + "_" + t 
        #vals_df.to_csv(output_dir + patID + "_inter_std.csv")
        all_df = all_df.append(vals_df, ignore_index=True)

all_df.to_csv(output_dir + "All_std.csv")
print("Finished")
